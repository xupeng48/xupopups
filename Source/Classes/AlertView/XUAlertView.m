...
#define ALERTVIEW_ANIMATE_DURATION  0.3f
...
- (void)buildSubViews {
    ...
    if (_title) {
        ...
        _titleLabel.textColor = [UIColor popupsColorWithString:@"#333333"];
    ...
    if (_message) {
        ...
        _messageLabel.textColor = [UIColor popupsColorWithString:@"#666666"];
        NSMutableParagraphStyle *paragraphStyle = [NSMutableParagraphStyle new];
        paragraphStyle.lineSpacing = 20 - _messageLabel.font.lineHeight;
        NSMutableDictionary *attributes = [NSMutableDictionary dictionary];
        [attributes setObject:paragraphStyle forKey:NSParagraphStyleAttributeName];
        _messageLabel.attributedText = [[NSAttributedString alloc] initWithString:_messageLabel.text attributes:attributes];
    ...
    if (_showCloseButton) {
        ...
        NSString *bundlePath = [[NSBundle bundleForClass:[self class]].resourcePath
                                stringByAppendingPathComponent:@"/XUPopups.bundle"];
        NSBundle *resource_bundle = [NSBundle bundleWithPath:bundlePath];
        UIImage *image = [UIImage imageNamed:@"popups_alertview_close"
...
- (void)layoutSubviews {
    ...
    if (_message) {
        ...
        [_messageLabel.attributedText enumerateAttribute:NSParagraphStyleAttributeName inRange:NSMakeRange(0, _messageLabel.text.length) options:NSAttributedStringEnumerationLongestEffectiveRangeNotRequired usingBlock:^(id  _Nullable value, NSRange range, BOOL * _Nonnull stop) {
            paragraph.paragraphSpacing = ((NSParagraphStyle *)value).paragraphSpacing;
        }];
        paragraph.lineSpacing = 20 - _messageLabel.font.lineHeight;
        CGSize size = [_messageLabel.text boundingRectWithSize:CGSizeMake(DEFAULT_CONTAINER_WIDTH - DEFAULT_LEFTRIGHT_PADDING * 2, CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName : _messageLabel.font, NSParagraphStyleAttributeName : paragraph} context:nil].size;
...
- (void)display:(void (^)(void))completion {
    ...
    if ([XUAlertDisplayManager sharedInstance].inDisplay) {
        [[XUAlertDisplayManager sharedInstance].alertViewArray addObject:self];
        return;
    }
    [XUAlertDisplayManager sharedInstance].inDisplay = YES;
    NSAssert([[NSThread currentThread] isMainThread], @"You must call this function on main thread");
    [self.subviews makeObjectsPerformSelector:@selector(removeFromSuperview)];
    [self buildSubViews];
    [window addSubview:self];
    [self becomeFirstResponder];
    [UIView animateWithDuration:ALERTVIEW_ANIMATE_DURATION
                          delay:0.0
         ...
         usingSpringWithDamping:1.0
          initialSpringVelocity:0.5
                     ...
                     animations:^{
                         ...
                         [XUAlertDisplayManager sharedInstance].inDisplay = NO;
                         if ([XUAlertDisplayManager sharedInstance].alertViewArray.count != 0) {
                             XUAlertView *alert = [[XUAlertDisplayManager sharedInstance].alertViewArray objectAtIndex:0];
                             [[XUAlertDisplayManager sharedInstance].alertViewArray removeObjectAtIndex:0];
                             [alert display];
                         }
                     }];
}
...
+ (void)showAlertWithIcon:(UIImage *)icon title:(NSString *)title attributeMessage:(NSAttributedString *)attributeMessage cancelTitle:(NSString *)cancelTitle cancelClick:(void (^)(void))cancelClick confirmTitle:(NSString *)confirmTitle confirmClick:(void (^)(void))confirmClick {
    XUAlertAction *cancelAction = [XUAlertAction actionHalfWidthWithTitle:cancelTitle style:XUAlertActionStyleNotRecommend onClick:cancelClick];
    XUAlertAction *confirmAction = [XUAlertAction actionHalfWidthWithTitle:confirmTitle style:XUAlertActionStyleRecommend onClick:confirmClick];
    XUAlertView *alert = [[XUAlertView alloc] initWithIconImage:icon headerImage:nil title:title message:@"" customView:nil];
    alert.messageLabel.attributedText = attributeMessage;
    [alert addAction:cancelAction];
    [alert addAction:confirmAction];
    [alert display];
}
