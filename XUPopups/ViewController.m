...
@property (nonatomic, copy) NSArray *sheetArray;
@property (nonatomic, copy) NSArray *toastArray;
...
- (void)viewDidLoad {
    ...
    self.sheetArray = @[@"sheet"];
    self.toastArray = @[@"toast"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        ...
        NSDictionary *dic = self.alertArray[indexPath.row];
        ...
        cell.textLabel.text = self.sheetArray[indexPath.row];
        return cell;
    } else {
        UITableViewCell *cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:NSStringFromClass([UITableViewCell class])];
        cell.textLabel.text = self.toastArray[indexPath.row];
        return cell;
    }
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        alertString = [NSString stringWithFormat:@"showAlert%ld", (long)indexPath.row + 1];
    } else if (indexPath.section == 1) {
        alertString = [NSString stringWithFormat:@"showSheet%ld", (long)indexPath.row + 1];
    } else {
        alertString = [NSString stringWithFormat:@"showToast%ld", (long)indexPath.row + 1];
    }
...
- (void)showAlert2 {
    ...
    lable1.textColor = [UIColor popupsColorWithString:@"#333333"];
    ...
    lable2.textColor = [UIColor popupsColorWithString:@"#999999"];
    ...
    lable3.textColor = [UIColor popupsColorWithString:@"#333333"];
    ...
    lable4.textColor = [UIColor popupsColorWithString:@"#999999"];
...
- (void)showAlert4 {
    ...
    loginTypeLabel.textColor = [UIColor popupsColorWithString:@"#333333"];
    ...
    [loginTypeString addAttribute:NSForegroundColorAttributeName
                            value:[UIColor popupsColorWithString:@"#999999"]
    ...
    userNameLabel.textColor = [UIColor popupsColorWithString:@"#333333"];
...
- (void)showAlert6 {
    ...
    label1.textColor = [UIColor popupsColorWithString:@"#666666"];
    ...
    label2.textColor = [UIColor popupsColorWithString:@"#FD5353"];
...
- (void)showAlert7 {
    [XUAlertView showAlertWithTitle:@"欢迎使用蜻蜓FM" message:@"第一时间获取更新提醒哦第一时间获取更新提醒哦第一时间获取更新提醒哦第一时间获取更新提醒哦第一时间获取更新提醒哦第一时间获取更新提醒哦第一时间获取更新提醒哦第一时间获取更新提醒哦第一时间获取更新提醒哦第一时间获取更新提醒哦第一时间获取更新提醒哦第一时间获取更新提醒哦第一时间获取更新提醒哦第一时间获取更新提醒哦第一时间获取更新提醒哦第一时间获取更新提醒哦" showClose:YES closeClick:nil confirmTitle:@"确 定" confirmStyle:XUAlertActionStyleRecommend confirmClick:^{
        NSLog(@"true1");
    }];
}

- (void)showAlert8 {
    [XUAlertView showAlertWithTitle:@"流量提醒1" message:@"当前无WiFi，是否允许流量收听？" cancelTitle:@"取消" cancelClick:^{
        NSLog(@"cancel");
    } confirmTitle:@"确定" confirmClick:^{
        NSLog(@"confirm");
    }];
    
    [XUAlertView showAlertWithTitle:@"流量提醒2" message:@"当前无WiFi，是否允许流量收听？" cancelTitle:@"取消" cancelClick:^{
        NSLog(@"cancel");
    } confirmTitle:@"确定" confirmClick:^{
        NSLog(@"confirm");
    }];
    
    [XUAlertView showAlertWithTitle:@"流量提醒3" message:@"当前无WiFi，是否允许流量收听？" cancelTitle:@"取消" cancelClick:^{
        NSLog(@"cancel");
    } confirmTitle:@"确定" confirmClick:^{
        NSLog(@"confirm");
    }];
...
- (void)showSheet1 {
    XUSheetView *sheet = [XUSheetView sheetViewWithCancel];
    [sheet addAction:[XUSheetAction actionWithTitle:@"删除" style:XUSheetActionStyleRed onClick:^{
        NSLog(@"删除");
    }]];
    [sheet addMaskViewAction:^{
        NSLog(@"hahah");
    }];
    [sheet display];
    
    [XUSheetView showSheetViewWithTitle:@"复制" style:XUSheetActionStyleDefault onClick:^{
        NSLog(@"复制");
    }];
}

- (void)showToast1 {
    [XU_HUDMAG displayMessage:@"this is a toast!!!" duration:2];
}
